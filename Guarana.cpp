...
void Guarana::kolizja(Organizm* inny, int oldX, int oldY) {
    getSwiat()->dodajLog(typeid(*this).name() + string(" zjada Guarane na polu (") + to_string(getX()) + "," + to_string(getY()) + ")");
    getSwiat()->dodajLog(typeid(*this).name() + string(" zwieksza sile o 3. Jest ona teraz rowna ") + to_string(getSila() + 3));
